import { shuffleArray, DIRECT_TWO } from "./MazeUtil.js";
...
class MazeKruskal {
  ...
  generate() {
    ...
    console.timeEnd('startgt')
    //并查集的跟primrandom差不多，但是不包含初始化边集合和随机化集合
    return wallData;
  }
  async asyncGenerate(callback) {
    const { row, col, totalCount, graphLists, matrix } = this;
    //并查集
    const wallData = [];
    let uf = new UnionFind(totalCount);
    for (let { begin, end } of graphLists) {
      ...
      if (bRoot !== eRoot) {
        ...
        typeof callback === "function" &&
          (await callback.call(this, wallx, wally));
